...
function ForgotPassword() {
  ...
  return (
    ...
    >
        {step < 4 &&
        <SoftBox ml={0.5} mt={1}>
            <SoftTypography className="text-xs" color="error" >
            STEP {step} of 3
            </SoftTypography>
        </SoftBox>
        }
        ...
        <SoftBox component="form" role="form" onSubmit={handleSubmit}>
            <SoftBox mb={1}>
                <SoftInput placeholder="Email" disabled={submitLogin} size="small" type="text"  name="username" value={formData1.username} onChange={handleChange}/>
            </SoftBox>
            <SoftBox mt={2} mb={1}>
                <SoftButton type="submit" size="small" variant="gradient" color="error" fullWidth className="rounded-pill">
                    submit
                </SoftButton>
            </SoftBox>
        </SoftBox>
        ...
        <SoftBox component="form" role="form" onSubmit={handleSubmitOTP}>
            <SoftBox mb={1}>          
                <SoftInput placeholder="Enter OTP" disabled={submitLogin} size="small" name="otp" value={formData2.otp} onChange={handleChange} />
            </SoftBox>
            <SoftBox mt={2} mb={1}>
                <SoftButton type="submit" size="small" variant="gradient" color="error" fullWidth className="rounded-pill">
                    submit
            ...
            <SoftBox mb={1}>
                <SoftButton onClick={() => setStep(1)} size="small" variant="gradient" color="secondary" fullWidth className="rounded-pill">
        ...
        <SoftBox component="form" role="form" onSubmit={handleSubmitPassword}>
            <SoftBox mb={1}>       
                <SoftInput placeholder="Set New Password" disabled={submitLogin} size="small" name="newpassword" value={formData3.newpassword} onChange={handleChange} />
            </SoftBox>
            <SoftBox mt={2} mb={1}>
                <SoftButton type="submit" size="small" variant="gradient" color="error" fullWidth className="rounded-pill">
                    submit
        ...
        {step == 4 &&
        <SoftBox mb={1}>         
            <SoftButton onClick={() => navigate("/authentication/sign-in")} size="small" variant="gradient" color="error" fullWidth className="rounded-pill">
                BACK TO LOGIN
            </SoftButton>
        </SoftBox>
        }
        {step < 4 &&
        <SoftBox display="flex"  justifyContent="center" textAlign={{ xs: "start", sm: "center" }}>
            <SoftTypography 
                py={1}
