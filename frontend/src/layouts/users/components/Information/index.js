...
function Information({ USER, HandleRendering, ReloadTable, allowDelete }) {
  ...
  const handleApprove = async (e) => {
    ...
    Swal.fire({
      ...
      text: "Are you sure you want to approve this driver? You won't be able to revert this!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        ...
        if (!token) {
          ...
          axios.get(apiRoutes.driverApprove, { params: { username }, headers })
            .then(response => {
              if (response.data.status == 200) {
                toast.success(`${response.data.message}`, { autoClose: true });
              } else {
            ...
            .catch(error => {
              setDeleteUser(false);
              toast.error("Cant approve user", { autoClose: true });
              passToErrorLogs(error, currentFileName);
            });
        }
      }
  ...
  const handleReject = async (e) => {
    ...
    Swal.fire({
      ...
      text: "Are you sure you want to reject this driver? You won't be able to revert this!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, arejectprove it!'
    }).then((result) => {
      if (result.isConfirmed) {
        setDeleteUser(true);
        if (!token) {
          ...
          axios.get(apiRoutes.driverReject, { params: { username }, headers })
            .then(response => {
              if (response.data.status == 200) {
                toast.success(`${response.data.message}`, { autoClose: true });
              } else {
            ...
            .catch(error => {
              setDeleteUser(false);
              toast.error("Cant reject user", { autoClose: true });
              passToErrorLogs(error, currentFileName);
            });
        }
      }
          ...
          <Grid mt={3} container spacing={0} alignItems="center" justifyContent="end">
            <Grid item xs={12} sm={4} md={2} pl={1}>
              <SoftBox mt={2} display="flex" justifyContent="end">
                ...
                {!allowDelete && USER.id_verified  == 0 && USER.access_level == 10 &&
                  <>
                    <SoftButton onClick={handleReject} variant="gradient" color="error" className="mx-2 w-100 text-xxs px-3 rounded-pill" size="small">
                      reject
                    </SoftButton>
                    <SoftButton onClick={handleApprove} variant="gradient" color="dark" className="mx-2 w-100 text-xxs px-3 rounded-pill" size="small">
                      approve
                    </SoftButton>
                  </>
                }
